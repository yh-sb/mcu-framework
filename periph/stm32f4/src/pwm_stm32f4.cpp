#include <limits>
#include <cassert>
#include "periph/pwm_stm32f4.hpp"
...
constexpr uint8_t const max_channels[timer_stm32f4::timers] =
{
    4, 4, 4, 4, 4, 0, 0, 4, 2, 1, 1, 2, 1, 1
};

constexpr uint32_t ccmr[pwm_stm32f4::pwm_channels][static_cast<uint8_t>(pwm_stm32f4::mode::inverted) + 1] = 
{
...
constexpr uint8_t gpio_af_list[timer_stm32f4::timers] =
{
    // TIM2 for STM32F446xx (AF0) not implemented
    0x01, 0x01, 0x02, 0x02, 0x02, 0x00, 0x00, 0x03, 0x03, 0x03, 0x03, 0x09, 0x09, 0x09
};
...
pwm_stm32f4::pwm_stm32f4(uint8_t timer, uint8_t channel, gpio_stm32f4 &gpio, enum mode mode):
    ...
    gpio(gpio)
{
    assert(timer >= 1 && timer <= timer_stm32f4::timers && timer_hw_mapping::timer[timer - 1]);
...
pwm_stm32f4::~pwm_stm32f4()
{
    timer_hw_mapping::timer[tim]->CR1 &= ~TIM_CR1_CEN;
    *timer_hw_mapping::rcc_en_reg[tim] &= ~timer_hw_mapping::rcc_en[tim];
}

void pwm_stm32f4::frequency(uint32_t frequency)
{
    assert(frequency > 0);
...
void pwm_stm32f4::duty_cycle(uint8_t duty_cycle)
{
...
void pwm_stm32f4::stop()
{
    timer_hw_mapping::timer[tim]->CR1 &= ~TIM_CR1_CEN;
}

void pwm_stm32f4::gpio_af_init(gpio_stm32f4 &gpio)
{
    ...
    gpio_reg->AFR[pin / 8] |= gpio_af_list[tim] << ((pin % 8) * 4);
}

void pwm_stm32f4::calc_frequency(uint8_t timer, uint32_t freq, uint16_t &presc, uint16_t &reload)
{
...
uint16_t pwm_stm32f4::calc_ccr(uint8_t timer, uint8_t duty_cycle)
{
    uint16_t ccr = 0;
