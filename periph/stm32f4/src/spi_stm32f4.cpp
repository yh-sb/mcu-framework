...
constexpr rcc::clk_source rcc_src[spis] =
{
    ...
    rcc::clk_source::apb2, rcc::clk_source::apb2, rcc::clk_source::apb2
};

constexpr uint8_t gpio_af[spis] =
{
    0x05, 0x05, 0x06, 0x05, 0x05, 0x05
};

spi_stm32f4::spi_stm32f4(uint8_t spi, uint32_t baudrate, enum cpol cpol, enum cpha cpha,
    ...
    read_buff(nullptr)
{
    ...
    assert(baud > 0);
    assert(write_dma.direction() == dma_stm32f4::direction::memory_to_periph);
    assert(write_dma.increment_size() == 8);
    assert(read_dma.direction() == dma_stm32f4::direction::periph_to_memory);
...
void spi_stm32f4::baudrate(uint32_t baudrate)
{
    assert(baudrate > 0);
...
void spi_stm32f4::cpol(enum cpol cpol)
{
    xSemaphoreTake(api_lock, portMAX_DELAY);
...
void spi_stm32f4::cpha(enum cpha cpha)
{
    xSemaphoreTake(api_lock, portMAX_DELAY);
...
void spi_stm32f4::bit_order(enum bit_order bit_order)
{
    xSemaphoreTake(api_lock, portMAX_DELAY);
...
spi::res spi_stm32f4::write(const void *buff, uint16_t size, gpio *cs)
{
    assert(buff);
    assert(size > 0);
...
spi::res spi_stm32f4::write(uint8_t byte, gpio *cs)
{
    xSemaphoreTake(api_lock, portMAX_DELAY);
...
spi::res spi_stm32f4::read(void *buff, uint16_t size, gpio *cs)
{
    assert(buff);
    assert(size > 0);
...
spi::res spi_stm32f4::write_read(const void *write_buff, void *read_buff, uint16_t size, gpio *cs)
{
    assert(write_buff);
    assert(read_buff);
    assert(size > 0);
...
void spi_stm32f4::gpio_af_init(gpio_stm32f4 &gpio)
{
    ...
    gpio_reg->AFR[pin / 8] |= gpio_af[spi] << ((pin % 8) * 4);
}

uint8_t spi_stm32f4::calc_presc(uint8_t spi, uint32_t baud)
{
...
void spi_stm32f4::on_dma_write(dma_stm32f4::event event)
{
...
void spi_stm32f4::on_dma_read(dma_stm32f4::event event)
{
    if(event == dma_stm32f4::event::half_complete)
    {
        return;
